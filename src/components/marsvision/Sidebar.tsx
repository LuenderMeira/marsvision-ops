import { LayoutGrid, CalendarDays, Users, Stethoscope, Package, Settings, Eye, CreditCard, Moon, Sun } from "lucide-react";
...
export function Sidebar({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <TooltipProvider>
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 flex-col border-r border-sidebar-border bg-sidebar md:flex">
        ...
        <div className="border-t border-sidebar-border p-4">
          <div className="flex items-center gap-3 px-1">
            ...
            <div className="min-w-0 flex-1 leading-tight"><div className="truncate text-sm font-medium text-sidebar-foreground">Dra. Silva</div><div className="truncate text-xs text-muted-foreground">Diretora clínica</div></div>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button type="button" variant="ghost" size="icon" onClick={toggleTheme} aria-label={isDark ? "Ativar tema claro" : "Ativar tema escuro"} className="shrink-0 text-muted-foreground hover:text-sidebar-foreground">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 flex h-16 items-center overflow-x-auto border-t border-border bg-sidebar px-1 shadow-sm md:hidden" aria-label="Navegação principal">
        {navItems.map((item) => { const active = value === item.id; return (
          <Button key={item.id} type="button" variant="ghost" onClick={() => onChange(item.id)} aria-label={item.label} className={cn("h-14 min-w-16 flex-1 flex-col gap-1 rounded-lg px-1 text-[10px] shadow-none", active ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-muted-foreground")}>
            <item.icon className="size-4" strokeWidth={2} /><span className="max-w-full truncate">{item.label}</span>
          </Button>
        ); })}
        <Button type="button" variant="ghost" onClick={toggleTheme} aria-label={isDark ? "Ativar tema claro" : "Ativar tema escuro"} className="h-14 min-w-14 flex-col gap-1 rounded-lg px-1 text-[10px] text-muted-foreground shadow-none">
          {isDark ? <Sun className="size-4" /> : <Moon className="size-4" />}<span>Tema</span>
        </Button>
      </nav>
    </TooltipProvider>
  );
